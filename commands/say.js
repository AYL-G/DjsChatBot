const { execute } = require("./weather");

//Syntax: say <channel>  <content>
module.exports = {
    name: "say",
    ...
    async execute (client, message, cmd, args, Discord){  
        ...
        // if the user running the command doesn't have permissions to mannage messages, stop running the code. ! means no or doesn't in js.

        let textChannel = message.mentions.channels.first() // the first channel that you tag
        if (!textChannel) return message.channel.send("Provide a channel for me to send the message in!");
        if(!args[0]) return message.channel.send('Provide a channel for me to send the message in!'); 
        // args are words or numbers after the command. The first word is args[0], and then args[1], and so on and so on.
        if(!args[1]) return message.channel.send('Provide a message to say!');
        if (!message.guild.channels.cache.has(textChannel.id)) return; 
        // here we're checking if the textChannel id provided is in the server where the message is being sent.
        message.delete() 
